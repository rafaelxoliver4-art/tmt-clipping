...
def enforce_covered_inclusion(report: dict, raw_rows: list, max_add: int = 10) -> dict:
    """Server-side safety net (added 2026-05-25): make sure every DIRECT-
    source row whose title contains a covered ticker / company name is in
    the curator's digest. Uses _is_direct_strict (no brand-prefix) to avoid
    matching things like "folha do es". For ambiguous aliases (Vivo/Tim/etc.),
    also requires a corporate qualifier OR ticker form in the title.
    """
    direct_info = _direct_source_names()
    ...
    for row in raw_rows:
        if counter["added"] >= max_add:
            break
        # Strict direct check — no brand-prefix (avoids "folha do es" type
        # false positives that the general _is_direct accepts).
