...
def _fetch_head(url: str) -> Optional[bytes]:
    """Fetch the first 30 KB of an article page (head + opening body is
    enough for almost all date markup)."""
    try:
        req = urllib.request.Request(url, headers={"User-Agent": _UA, "Range": "bytes=0-30000"})
