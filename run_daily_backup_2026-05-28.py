...
def main():
    ...
    # Article-time enrichment: for items with date-only timestamps (from HTML
    # sources without time markup in the listing), fetch the article page
    # and extract HH:MM from <meta>/<time> tags. Adds ~30-60s.
    try:
        ...
        all_items = [it for sec, items in report.items()
                     ...
                     for it in items]
        if all_items:
            ec_time = article_time_enricher.enrich(all_items)
            print(f"  Article-time enrich: {ec_time['updated']} updated, "
                  f"{ec_time['checked']} checked, {ec_time['skipped']} skipped")
    except Exception as e:
        print(f"  [WARN] article-time enrich failed: {e}")
