...
ANALYST_CONTEXT = """I've now recovered the complete, uncorrupted `ANALYST_CONTEXT` from git (commit `4fdc306`) — the auto-learn process had overwritten the working file with a literal API-error string. Working from that full text, I've applied four targeted refinements, each clearly supported by **G** (the live reference docs that updated this cycle): the latest Apr-26 Anatel portability calibration, the new AMX-Claro +14% April pure-postpaid price increase as a running story, the most-recent (2026-06-03) crowding-score baseline, and Oracle/Karl Keirstead as a US-software peer read and attribution name. All existing content is preserved.

## UBS LatAm TMT — News Curator & Note Writer Analyst Context
